...
import agents.rvt.rvt.utils.peract_utils as peract_utils
import agents.rvt.rvt.mvt.utils as mvt_utils
import agents.rvt.rvt.utils.rvt_utils as rvt_utils
import agents.rvt.rvt.libs.peract_colab.arm.utils as arm_utils

from agents.rvt.rvt.mvt.augmentation import apply_se3_aug_con, aug_utils
from agents.rvt.rvt.libs.peract_colab.arm.optim.lamb import Lamb
from yarr.agents.agent import ActResult
from agents.rvt.rvt.utils.dataset import _clip_encode_text
from agents.rvt.rvt.utils.lr_sched_utils import GradualWarmupScheduler
...
class RVTAgent:
    def __init__(
        self,
        network: nn.Module,
        skill_manager: nn.Module,
        visual_aligner: nn.Module,
    ...
    def update(
        ...
        # print(replay_sample["low_dim_state"].shape) # 4 for right, 12 for left
        ...
        obs, pcd, rgb = peract_utils._preprocess_inputs(replay_sample, self.cameras) # obs = [rgb, pcd]

        ####################AnyBimanual######################
        rgb_right, rgb_left, pcd_right, pcd_left = self.visual_aligner(rgb, pcd)
        right_skill = self.skill_manager(rgb_right,pcd_right)
        left_skill = self.skill_manager(rgb_left,pcd_left)
        if replay_sample["low_dim_state"].shape == 4:
            rgb_ = rgb_right
            pcd_ = pcd_right
            lang_goal_embs = torch.cat((right_skill,lang_goal_embs),dim=1)
        else:
            rgb_ = rgb_left
            pcd_ = pcd_left
            lang_goal_embs = torch.cat((left_skill,lang_goal_embs),dim=1)
        obs_ = []
        for i in range(len(rgb)):
            obs_.append([rgb_[i],pcd_[i]])
        #####################################################
        ...
        with torch.no_grad():
            ...
            ####################AnyBimanual######################
            pc_, img_feat_ = rvt_utils.get_pc_img_feat(
                obs_,
                pcd_,
            )
            ####################AnyBimanual######################
            ...
            ####################AnyBimanual######################
            pc_, img_feat_ = rvt_utils.move_pc_in_bound(
                pc_, img_feat_, self.scene_bounds, no_op=not self.move_pc_in_bound
            )
            ####################AnyBimanual######################
            ...
            for i in range(len(pc)):
                pc_cat = torch.cat((pc_[i],pc[i]),dim=0)
                pc_new.append(pc_cat)
                img_cat = torch.cat((img_feat_[i],img_feat[i]),dim=0)
                img_feat_new.append(img_cat)
            pc = pc_new
            img_feat = img_feat_new
            ####################AnyBimanual######################
    ...
    def act(
        ...
        obs, pcd,rgb = peract_utils._preprocess_inputs(observation, self.cameras)
        ####################AnyBimanual######################
        rgb_right, rgb_left, pcd_right, pcd_left = self.visual_aligner(rgb, pcd)
        right_skill = self.skill_manager(rgb_right,pcd_right)
        left_skill = self.skill_manager(rgb_left,pcd_left)
        if observation["low_dim_state"].shape == 4:
            rgb_ = rgb_right
            pcd_ = pcd_right
            lang_goal_embs = torch.cat((right_skill,lang_goal_embs),dim=1)
        else:
